...
class RoleTower {
    ...
    run(tower) {
        ...
        if (remoteTarget) {
            ...
            try {
                if ('Invader' !== remoteTarget.owner.username) {
                    Game.notify([tower.structureType, tower.room.name].concat(['tower attacking', JSON.stringify(remoteTarget.owner), util.bodyToShortString(remoteTarget.body)]));
                }
            } catch(e) {
            ...
            let stat = _.get(Memory.stats,s,1);
            _.set(Memory.stats, s, (stat ? 1 : stat + 1));
        } else {
